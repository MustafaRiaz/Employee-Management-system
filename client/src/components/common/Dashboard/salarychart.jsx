...
export const SalaryChart = ({ balancedata }) => {
    ...
    if (balancedata?.balance?.length) {
        for (let index = 0; index < balancedata.balance.length; index++) {
            const entry = balancedata.balance[index];
            chartData.push({
                month: entry?.expensemonth ?? "Unknown",
                SalriesPaid: entry?.totalexpenses ?? 0,
                AvailableAmount: entry?.availableamount ?? 0,
            });
        }
    }
    ...
    if (chartData.length >= 2) {
        ...
        if (
            typeof last.AvailableAmount === "number" &&
            typeof prev.AvailableAmount === "number" &&
            prev.AvailableAmount !== 0
        ) {
